    ...
    while not closed() and not (max_score >= MAX_SCORE and state == "next_round" and time() > reset_time):
        ...
        if not p1_lost:
            ...
            if len(p1_queue) >= 4:
                ...
                #
                if len(p1_queue) >= 6 and has_collided(p1_x, p1_y, p1_queue[-6], p1_queue[-5], p1_queue[:-4]):
                    p1_lost = True
                #
            ...
            if e_lost[i] == False:
                ...
                while angle <= e_headings[i] + 0.6 * pi:
                    (hits, x, y) = closestCollision(e_queues[i][-2], e_queues[i][-1],
                                                    ...
                                                    [extended_p1_queue, most_e_queue, BOUNDARY] + others)
                    ...
                    if hits and found == False:
                        ...
                        if dist2(e_queues[i][-2], e_queues[i][-1], x, y) > dist2(e_queues[i][-2], e_queues[i][-1],
                                                                                 best_x, best_y):
                            best_x = x
                            best_y = y
                            best_angle = angle
        ...
        for j in range(len(e_scores)):
            setColor(e_colors[j])
            text((j + 1) * getWidth() / (len(e_scores) + 1), getHeight() - 20, e_names[j] + ": " + str(e_scores[j]),
                 "w")
